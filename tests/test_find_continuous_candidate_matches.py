...
from mwe_detector.utils import find_continuous_candidate_matches


def test_empty_input():
    assert find_continuous_candidate_matches([], []) == []


def test_no_matching_lemmas():
    lemmas = ["test1", "test2"]
    tokens = ["test3", "test4"]
    assert find_continuous_candidate_matches(lemmas, tokens) == []


def test_single_matching_lemma_single_occurrence():
    lemmas = ["test1"]
    tokens = ["test1", "test2"]
    assert find_continuous_candidate_matches(lemmas, tokens) == [(0,)]


def test_single_matching_lemma_multiple_occurrences():
    lemmas = ["test1", "test1"]
    tokens = ["test1", "test1", "test2"]
    assert find_continuous_candidate_matches(lemmas, tokens) == [(0, 1)]
...
def test_multiple_matching_lemmas():
    ...
    expected_result = [(0, 1)]
    assert find_continuous_candidate_matches(lemmas, tokens) == expected_result
...
def test_multiple_continuous_matches():
    lemmas = ["test1", "test2"]
    tokens = [
        "test1",
        "test2",
